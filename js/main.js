window.onload = function() {
	...
	//calling ipinfo.io to get the location
	...
	function httpReqWeatherAsync(url, callback) {
		...
		httpReqWeather.onreadystatechange = function() {
			if(httpReqWeather.readyState == 4 && httpReqWeather.status == 200) {
				...
				var tempCelu = Math.round(temperature - 273)
				...
				tempElement.innerHTML = `${tempCelu}<i id="icon-thermometer" class="wi wi-thermometer"></i>`	;
